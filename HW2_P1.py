...
# Train and fine-tune a decision tree model with the Titanic training data
...
# Predicting the training accuracy for tuned DT model
Y_pred = dtc.predict(X_test)
acc_dtc = round(dtc.score(X_train, Y_train) * 100, 2)
acc_dtc

# Perform 5-fold cross validation for DT
cv_results = cross_validate(dtc, X_train, Y_train, scoring='accuracy', cv=5)
acc_dtc_cv = round(cv_results['test_score'].mean() * 100, 2)
print(acc_dtc_cv)

# Predicting the training accuracy for tuned random forest model
...
# Perform 5-fold cross validation for RFC
cv_results1 = cross_validate(rfc, X_train, Y_train, scoring='accuracy', cv=5)
acc_rfc_cv = round(cv_results1['test_score'].mean() * 100, 2)
print(acc_rfc_cv)
